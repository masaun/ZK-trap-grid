...
function poseidonHash2(left, right) {
    ...
    return (0, poseidon_lite_1.poseidon2)([leftBigInt, rightBigInt]);
}
/**
 * Generate a simple trap grid (all zeros except specified traps)
 */
function generateTrapGrid(traps = []) {
 ...
 */
function computeTrapCommitment(trapValue, leafIndex) {
    return poseidonHash2(BigInt(trapValue), BigInt(leafIndex));
}
/**
 * Build Merkle tree from trap commitments
 */
function buildMerkleTree(grid) {
    // Level 0: Compute commitments for all leaves
    let currentLevel = grid.map((trap, idx) => computeTrapCommitment(trap, idx));
